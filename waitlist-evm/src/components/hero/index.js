...
const Hero = () => {
  ...
  const connect = async () => {
    if (window.ethereum) {
      try {
        ...
        const signed_msg = await Web3Token.sign(async msg => await signer.signMessage(msg), address, '1d');
  ...
  const getButton = () => {
    switch(buttonSetting) {
      case 1:
        return (
            <Fill onClick={connect}>{'Connect Your Wallet'}</Fill>
        )
      case 2:
        return (
            <Fill href="/submit">{'Join Waitlist'}</Fill>
        )
      default:
        return (
            <Fill onClick={connect}>{'Install MetaMask!'}</Fill>
        )
    }
  ...
  return (
    <Container>
      <Content>
        <Title>AIR DROP WAITLIST</Title>
        <Description>
          Authenticated: { isLoggedin ? "True" : "False" }<br />
          Address: { address }
        </Description>
        <Section>
          <Outline to="" onClick={logout}>Learn More</Outline>
          { getButton() }
        </Section>
      </Content>
    </Container>
